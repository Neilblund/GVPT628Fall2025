...
checkFPL<-function(years, household_sizes, incomes, states='US'){
  ...
  completed_url <- paste0(base_url,"/",years, '/', states, '/', household_sizes)
...
Obviously, this isn't the only way to solve this problem, and there may be scenarios where it actually makes more sense to separate this process into more than one function. For instance: if you were expecting to get new data throughout the year, it might make more sense to make a function that created an FPL lookup table and then stored the results, along with a separate function that matched that table to new household data.
